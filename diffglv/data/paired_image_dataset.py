...
from diffglv.data.data_util import paired_center_crop
from diffglv.utils.transforms import paired_random_crop
...
class MultiPairedImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.task == 'CAR':
            ...
            # img_gt = img_gt[:,:,0,None]
            # img_lq = img_lq[:,:,0,None]

        elif self.task == 'Color Denoising':
            gt_path = self.paths[index]['gt_path']
            lq_path = gt_path
            img_bytes = self.file_client.get(gt_path, 'gt')
            ...
            # bicubic
            img_lq = cv2.resize(img_lq, (img_lq.shape[1]*scale, img_lq.shape[0]*scale), interpolation=cv2.INTER_CUBIC)
        ...
        return {'lq': img_lq, 'gt': img_gt, 'lq_path': lq_path, 'gt_path': gt_path, 'task': self.task}
